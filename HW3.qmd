...
## 0.3 crs check to start

Let's ensure everything is in the same CRS and what CRS we are in to start
...
```{r}
# first check extent (need same extent to do raster math)
st_bbox(d1_stars)
...
Now let's re-project the cropped blackout dataset to EPSG:3083 (NAD83 / Texas Centric Albers Equal Area)
