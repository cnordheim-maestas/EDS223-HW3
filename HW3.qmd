...
Note: st_bbox needs to be converted (vectorized) into layer
...
I dont even know what this means...do we need to do more calculations?? what is this asking? Our workflow didnt make anthing that fits this
...
tmap_save(m_homes_lostpower, filename = "figs/fig2.png", width = 8, height = 6, dpi = 300)
```
...
```{r}
fig3 <- tm_basemap("OpenStreetMap") +
  tm_graticules() +
  tm_shape(se, bbox = st_bbox(homes_blackout_yn)) +   
    tm_borders(col = "purple") +
  tm_shape(homes_blackout_yn) +
    tm_polygons(fill = "blackout", # column name with the values
              fill.legend = tm_legend(title = "Blackout experienced", 
                                      position = tm_pos_out("right"))) +
  tm_title("houston homes and census tracts")
fig3

tmap_save(fig3, filename = "figs/fig3.png", width = 8, height = 6, dpi = 300)
```
...
income_blackout_fig
ggsave(
  filename = "figs/income_blackout_fig.png",  # or .pdf / .tiff / .jpg
